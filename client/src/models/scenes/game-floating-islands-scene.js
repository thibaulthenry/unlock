...
export default class GameFloatingIslandsScene extends Scene {
  ...
  constructor() {
    ...
    this.axolotlMap = new Map()
    this.built = false
    this.islandStateMap = new Map()
  ...
  create() {
    ...
    this.axolotl = new Axolotl(
        this,
        PhaserMath.Between(200, this.sceneWidth - 200),
        -100,
        'axolotl',
  ...
  createIslands(packet) {
    ...
    packetArray.sort((a, b) => {
      if (a.coordinates.y < b.coordinates.y) return -1
      if (a.coordinates.y > b.coordinates.y) return 1
      if (a.coordinates.x > b.coordinates.x) return 1
      if (a.coordinates.x < b.coordinates.x) return -1
      return 0
    })
    ...
    switch (packet.label) {
      case PacketLabels.SERVER_SCENE_DATA:
        new PacketServerSceneData(packet).receive(
            ...
            p => {
              if (this.built) {
                this.updateIslandSprites(p)
              } else {
                this.createIslands(p)
              }
            },
        )
        break
      case PacketLabels.SERVER_SCENE_MOVEMENT:
        new PacketServerSceneMovement(packet).receive(
            SceneKeys.GAME_FLOATING_ISLANDS,
            p => SceneUtils.handleServerAxolotlMovement(p, this.axolotlMap),
        )
        break
    }
  }
